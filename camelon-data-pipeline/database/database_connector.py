...
def insert_data_into_table(conn, data):
    ...
    for index, row in data.iterrows():
        print(str(index+1) + " of " + str(len(data)) + " news")
        # Check if record already exists in the table
        sql_select = "SELECT COUNT(*) FROM news_metadata WHERE News_id = %s"
        cursor.execute(sql_select, (row['news_id'],))
        result = cursor.fetchone()
        # If record doesn't already exist, insert it into the table
        if result[0] == 0:
            print("Insert " + row['news_id'] + " into table")
